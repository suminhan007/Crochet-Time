...
const ColorCardList:React.FC<Props> = ({
    ...
    const [pixelData,setPixelData] = useState<{id:string,img_url:string,size?:string}[]>([]);
    const [loading,setLoading] = useState(true);
    const fetchLatestImage = async () => {
        const {data:{user}} = await supabase.auth.getUser();
        const userId = user?.id;
        const { data:ImgData, error } = await supabase
            .from('pixelCard') // 替换为你的表名
            .select('id,img_url, size')
            .eq('user_id',userId)
        ...
        if (error) {
            ...
            const { data: UrlData, error:UrlError } = await supabase
                .storage
                .from('CroKnitTime')
                .createSignedUrls(ImgData?.map(i => `pixelCards/${i.img_url}`), 60)
            if(UrlError){
    ...
    const handleDeleteColorCard = async () => {
        const response = await supabase.from('pixelCard').delete().eq('id',selectedCard);
    ...
    const handleDownloadColorCard = async (url:string) => {
        const downloadUrl =url.split('?token=')[0].split('CroKnitTime/pixelCards/')[1];
        const response = await supabase.storage.from('CroKnitTime').download(`pixelCards/${downloadUrl}`);
        if(response.error){
    ...
    return (
        <>
            {loading ? <div className={'width-100 flex-1 flex both-center'}>
                <LandLoading />
            </div> : (pixelData && pixelData?.length >0) ? <div className={'flex column gap-16'}>
                {pixelData?.map(i =>  <div key={i.img_url} className={'relative flex items-center gap-12 p-12 border radius-8 border-box'}>
                ...
                <LandState type={'empty'} title={<>暂无色卡, <LandLink
                    onClick={() => navigate('/type=tools-pixelDrawer')}>前往制作</LandLink></>}/>
            </div>}
            {/*发布确认弹窗*/}
            <LandDialog
                title={'确认发布该像素卡到社区？'}
            ...
            >
                <LandAlert title={'色卡发布到社区后将成为公开像素卡，允许所有人查看、下载。'}/>
            </LandDialog>
            {/*删除确认弹窗*/}
            <LandDialog
                title={'确认删除该像素卡？'}
