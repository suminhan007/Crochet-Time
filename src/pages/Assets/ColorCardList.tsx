...
type Props = {
    loading: boolean;
    data?: { id:string,img_url:string,origin_img_url:string,colors: { id:number,value:string }[],description:string }[];
}
    ...
    return (
        <>
            {loading ? <div className={'width-100 flex-1 flex both-center'}>
                <LandLoading />
            </div> : (data && data?.length >0) ? <div className={'flex column gap-16'}>
                {data?.map(i =>  <div key={i.img_url} className={'relative flex items-center gap-12 p-12 border radius-8 border-box'}>
