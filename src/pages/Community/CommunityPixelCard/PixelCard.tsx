...
type Props = {
    item:{id:string,img_url:string,size:string,user:{username:string,avatar_url:string,is_official?:boolean}};
}
const PixelCard:React.FC<Props> = ({
            ...
            if (StaredRes.error || !StaredRes.data) {
                ...
                if (ids.length > 0 && ids.includes(item.id)) {
                    ...
                    const res = await supabase.from('staredCard').insert({
                        card_id: item.id,
                        user_id: user?.id,
                        img_url: item.img_url.split('/CroKnitTime/pixelCards/')[1]?.split('?token=')[0],
                        type: 'pixelCards'
                    })
                    if (res.error) {
                    } else {
    ...
    return <div  className={'flex column gap-8'}>
        ...
        <div className={'fs-12 color-gray-4'}>{item.size}</div>
