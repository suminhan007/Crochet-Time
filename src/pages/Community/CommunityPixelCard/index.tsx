...
const CommunityPixelCard:React.FC = () => {
    ...
    const fetchLatestImage = async () => {
        ...
        if (error) {
            ...
            const { data: UrlData, error:UrlError } = await supabase
                .storage
                .from('CroKnitTime')
                .createSignedUrls(communityData?.map(i => `pixelCards/${i.img_url}`), 60)
            if(UrlError){}else{
    ...
    const handleDownloadColorCard = async (url:string) => {
        ...
        const response = await supabase.storage.from('CroKnitTime/pixelCards').download(downloadUrl);
        if(response.error){
        ...
        <>
            {loading ? <div className={'width-100 height-100 flex-1 flex both-center'}>
                ...
                <LandState type={'empty'} title={<>暂无公开像素卡, <LandLink
                    ...
                    onClick={() => navigate('/type=repository')}>发布我的像素卡</LandLink></>}/>
            </div>}
